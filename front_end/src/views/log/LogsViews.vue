<template>
    <div class="flex flex-col gap-6 p-6 rounded-xl border shadow-sm bg-card">
        ...
        <div class="rounded-lg overflow-hidden">
            <Table :columns="columns" :data="Logs" :isPagination="true" :isSearchable="true" :is-filter-select="true"
                filter-select-column="action" filter-select-label="Activity" :filter-select-options="[
...
export default {
    ...
    methods: {
        async fetchLog() {
            try {
                const response = await useLogStore.getAll();
